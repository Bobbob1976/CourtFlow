-- NUCLEAR REBUILD V2 (038)
-- Handles "function not unique" errors by dynamically dropping ALL versions of the function.

-- 1. DROP ALL VERSIONS OF THE FUNCTION (Dynamic Loop)
DO $$
DECLARE
    func_record RECORD;
BEGIN
    FOR func_record IN 
        ...
        FROM pg_proc 
        WHERE proname = 'create_atomic_booking'
    LOOP
...
-- 5. RECREATE MATCH PLAYERS TABLE
...
-- 8. RECREATE TRIGGER FUNCTION & TRIGGER
