-- SMART TRIGGER FIX (035)
-- Uses DISABLE TRIGGER USER to safely bypass triggers without needing superuser permissions.

-- 1. DISABLE USER TRIGGERS (Safe way to silence the old trigger)
ALTER TABLE bookings DISABLE TRIGGER USER;

-- 2. DROP THE OLD TRIGGER AND FUNCTIONS
...
-- 3. FIX SCHEMA AND DATA (Now safe from trigger interference)
DO $$
BEGIN
...
-- 6. POPULATE TIME_RANGE (Manually, since triggers are disabled)
...
-- 7. RECREATE ATOMIC BOOKING FUNCTION
